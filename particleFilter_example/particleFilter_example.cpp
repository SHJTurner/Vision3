...
int main (int argc, char * const argv[]) {
  ...
  particleFilter pFilter = particleFilter(650,650,650);
  for(;;) {
    ...
    continue;
      ...
      measurement(0) = mouse_info.x; //save messurement
      measurement(1) = mouse_info.y; //save messurement

      cv::Point measPt(measurement(0),measurement(1)); //only used to plot mouse pos
      mouseV.push_back(measPt); //only used to draw line
    ...
    drawCross(partPt , cv::Scalar(255,0,255), 2);
      ...
      cv::Point3f tmp = pFilter.filter(mouse_info.x,mouse_info.y,1);
      cv::Point statePt = cv::Point(tmp.x,tmp.y); //(condens->State[0], condens->State[1]);
