...
cv::Point3f particleFilter::filter(float x, float y, float z)
{
    ...
    {
        float diffX = (x - this->condens->flSamples[i][0])/this->maxRange[0]; //maxRange[0] should be x range. Works as ling minRange[0] = 0
        float diffY = (y - this->condens->flSamples[i][1])/this->maxRange[1]; //maxRange[1] should be y range
        float diffZ = (z - this->condens->flSamples[i][2])/this->maxRange[2]; //maxRange[1] should be z range
